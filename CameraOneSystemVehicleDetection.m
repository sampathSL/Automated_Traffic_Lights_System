function [Status,Exception,cacledMinObjY,numberOfVehicles] = CameraOneSystemVehicleDetection(numberOfFrames,threshold,unwantedBlobSize,trackMinBlobSize,backgroundImage,laneMarginData,isVehicleDrivingLeftSide)
    %laneMarginData [lllX(LanelowerleftXvalue) llrX lulX lurX lllY llrY
    %lulY lurY]
    %isVehicleDrivingLeftSide - camera point of view - sit infront of
    %camera
    ...
    try
        ...
        currentFrames = vidObjectOne.FramesAcquired;
        ...
        while(vidObjectOne.FramesAcquired <= ( currentFrames + numberOfFrames))

            % Get the snapshot of the current frame
            currentImage = getsnapshot(vidObjectOne);
            ...
            %image show;
            subplot(2,2,1),imshow(currentImage);
            hold on
            pause(0.001);
